	...
	opts = function()
		...
		vim.api.nvim_create_autocmd({ "BufEnter", "BufWinEnter" }, {
			callback = function()
				...
				local hide_tabline = ft == "alpha" or ft == "oil"

				vim.opt.showtabline = hide_tabline and 0 or 2
				require('lualine').hide({
					place = { 'tabline' },
					unhide = not hide_tabline,
				})
			end,
		})
				...
				lualine_x = {
					{
						"diff",
						diff_color = {
							added = { bg = catppuccin.crust, fg = catppuccin.green },
							modified = { bg = catppuccin.crust, fg = catppuccin.peach },
							removed = { bg = catppuccin.crust, fg = catppuccin.red },
						},
					}
				},
			},
